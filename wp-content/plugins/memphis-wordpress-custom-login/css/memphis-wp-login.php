...
#login { width: <?php echo $custom_width; ?>%; }
...
#login h1 a { <?php echo $custom_bgimage[imageurl] == "" ? '' : 'background: url('.$custom_bgimage[imageurl].') no-repeat;';?> margin:10px 10px; padding:0; width: <?php echo $custom_bgimage[width];?>px; height: <?php echo $custom_bgimage[height];?>px;}
#login label, .login #nav, #reg_passmail, #loginform p { color: <?php echo $custom_textcolor; ?>;}
.login #nav a, .login #backtoblog a {<?php echo $remove_text_shadow == 1 ? 'text-shadow: none !important;' : ''; echo $custom_linkcolor_normal == 1 ? 'color: '.$custom_linkcolor_normal.' !important;' : ''; ?> !important; }
